...
driver.get('https://shopee.sg/abc-i.1507528958.40158187769')
...
if found_products and products_data:
    csv_filename = 'shopee_single_product_detail.csv'
    with open(csv_filename, 'w', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=products_data[0].keys())
        writer.writeheader()
        writer.writerows(products_data)
    print(f"\nSaved {len(products_data)} products to '{csv_filename}'")
    
    with open('shopee_products.json', 'w', encoding='utf-8') as f:
        json.dump(products_data, f, indent=2, ensure_ascii=False)
    print("Also saved to 'shopee_products.json'")
else:
    print("No products found. Try scrolling more or waiting longer.")
